...
public class AccelerometerCollectionService extends Service {
    private Socket mSocket;
    public static final String HEADER = "date,time,x (m/s),y (m/s), z(m/s)";
    public static final String INTENT_FILE = "intent_file";

    public static void start(Context context, File file) {
        Intent intent = new Intent(context, AccelerometerCollectionService.class);
    ...
    private BandAccelerometerEventListener mBandAccelerometerEventListener = new BandAccelerometerEventListener() {
        ...
        public void onBandAccelerometerChanged(BandAccelerometerEvent bandAccelerometerEvent) {
            String contents = bandAccelerometerEvent.getAccelerationX() + "," + bandAccelerometerEvent.getAccelerationY() + "," + bandAccelerometerEvent.getAccelerationZ();
            Log.d(this.getClass().getSimpleName(), contents);
            mSocket.emit("new message", contents);
    ...
    public void onCreate() {
        ...
        boolean drawing = SettingsActivity.getBoolean(this, "draw_preference", false);
        if(drawing == true)
        {
            try {
                mSocket = IO.socket("http://bitsandbotsandboots.azurewebsites.net");
            } catch (URISyntaxException e) {}

            mSocket.connect();
            mSocket.open();

            mSocket.emit("add user", "" + SettingsActivity.getString(this, "pref_id", "Bit Bot W/ Boots"));
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mBandClient.getSensorManager().registerAccelerometerEventListener(mBandAccelerometerEventListener, SampleRate.MS32);
